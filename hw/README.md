# Brain MRI Prediction: Age/Sex Regression on UKB and CN/MCI/AD Classification on ADNI
...
```bash
cd /mnt/data/haoyang/workspace/hw/FDU-computational-neuroscience-26spring/hw
tar -xzf dataset/UKB_test20_release.tar.gz -C dataset
tar -xzf dataset/ADNI_test20_release.tar.gz -C dataset
```
...
输出：`outputs/UKB_test20_release.csv`
...
输出：`outputs/ADNI_test20_release.csv`

- Age 使用 `configs/sfcn_ukb_age_finetune_data_aug_seed3.yaml`，Sex 使用 `configs/sfcn_ukb_sex_finetune_seed3.yaml`，ADNI 使用 `configs/rootstrap_adni_finetune_data_aug_seed3.yaml`
- 每个任务读取 3 个 seed x 5 folds，共 15 个 checkpoint
- UKB 输出格式为 `eid,age,sex`；ADNI 输出格式为 `eid,label`

ADNI test20 的 Rootstrap 15 模型概率排查：

- 类别顺序为 `AD,MCI,CN`
- 单个 checkpoint 的预测分布有波动，但没有全部偏向同一类；15 模型 logits 平均后的最终分布为 `AD=7, MCI=3, CN=10`
- ensemble 平均概率为 `AD=0.350, MCI=0.141, CN=0.509`
- 20 个样本中没有 `margin < 0.15` 的低置信样本，说明当前结果主要来自模型集成后的强判断，而不是三类概率接近导致的随机摆动

| eid | pred | p_AD | p_MCI | p_CN | margin | vote_AD | vote_MCI | vote_CN |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 005_S_0814 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 15 | 0 | 0 |
| 021_S_0753 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 14 | 1 | 0 |
| 023_S_0625 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 14 | 1 | 0 |
| 024_S_5054 | MCI | 0.0096 | 0.8189 | 0.1716 | 0.6473 | 5 | 5 | 5 |
| 029_S_0836 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 14 | 1 | 0 |
| 031_S_4042 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 1 | 14 |
| 002_S_0413 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 0 | 15 |
| 005_S_0553 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 1 | 14 |
| 005_S_0610 | AD | 0.9900 | 0.0100 | 0.0000 | 0.9799 | 8 | 6 | 1 |
| 006_S_0731 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 0 | 15 |
| 011_S_4105 | CN | 0.0000 | 0.0008 | 0.9992 | 0.9984 | 0 | 5 | 10 |
| 022_S_4196 | MCI | 0.0000 | 1.0000 | 0.0000 | 1.0000 | 1 | 11 | 3 |
| 036_S_5248 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 0 | 15 |
| 011_S_0326 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 13 | 2 | 0 |
| 031_S_4721 | MCI | 0.0000 | 1.0000 | 0.0000 | 1.0000 | 0 | 12 | 3 |
| 041_S_4051 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 0 | 15 |
| 041_S_4874 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 0 | 0 | 15 |
| 057_S_1007 | AD | 1.0000 | 0.0000 | 0.0000 | 1.0000 | 15 | 0 | 0 |
| 067_S_0284 | CN | 0.0000 | 0.0000 | 1.0000 | 1.0000 | 1 | 3 | 11 |
| 067_S_0607 | CN | 0.0000 | 0.0001 | 0.9999 | 0.9998 | 0 | 5 | 10 |
